...
\begin{enumerate}
    \item \definition{Direct-Mapped Cache}. It is the simplest and most intuitive. Each block from memory \textbf{can go in only one location} in the cache.
    \begin{equation}
    ...
    \item \definition{Fully Associative Cache}. A block from memory \textbf{can be placed in any cache line}. There is \hl{no index in the address} (no needed), and the mapping rule is a \textbf{full \texttt{TAG} comparison across all cache lines}. This means:
    \begin{itemize}
    ...
    \item \definition{N-Way Set-Associative Cache}. A compromise between direct-mapped and fully associative.
    \begin{itemize}
