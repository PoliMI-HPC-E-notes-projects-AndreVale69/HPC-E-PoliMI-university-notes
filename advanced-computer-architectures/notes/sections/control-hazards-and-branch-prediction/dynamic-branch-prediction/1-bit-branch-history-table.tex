\subsubsection{1-bit Branch History Table}\label{sec:1-bit-bht}\label{def:branch-history-table}

In general, the \definition{Branch History Table (BHT)}, or \definition{Branch Prediction Buffer}, is a \textbf{dynamic hardware structure} that \textbf{predicts branch outcomes based on recent execution behavior}. Specifically, it is a Program Counter (PC)-indexed table that associates branch instructions with a \textbf{prediction state}, and dynamically predicts whether a branch will be \emph{taken} or \emph{not taken} using small finite-state machines (FSMs), such as 1-bit or 2-bit saturating counters.

\highspace
The BHT can be seen as a \textbf{cache-like structure} that maps branch addresses to prediction states. Rather than storing an explicit sequence of past outcomes, each entry \textbf{implicitly encodes the recent behavior of the branch} through its FSM state, which evolves according to previous branch resolutions. This implicit history allows the processor to adapt predictions dynamically and to tolerate occasional anomalies, such as loop exits.

\highspace
The \hl{simplest version uses 1 bit per branch} to remember whether the branch was \textbf{recently taken or not taken}. For this reason, it is called a \definition{1-bit Branch History Table (1-bit BHT)}. It operates at runtime and uses a \textbf{Final State Machine (FSM) with 1-bit history}:
\begin{itemize}
