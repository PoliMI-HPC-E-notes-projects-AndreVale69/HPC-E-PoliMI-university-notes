...
\begin{itemize}
    ...
    \item \phantomsection\label{def:dynamic-branch-prediction} \definition{Dynamic Branch Prediction Techniques}. Unlike static methods, dynamic prediction uses \textbf{hardware mechanisms to observe past branch behavior at runtime} and make predictions accordingly. The \textbf{prediction adapts to actual program execution}, making it more effective for applications with \textbf{complex or data-dependent control flow}. This method can dynamically switch its guess depending on the \emph{branch history}.
\end{itemize}
