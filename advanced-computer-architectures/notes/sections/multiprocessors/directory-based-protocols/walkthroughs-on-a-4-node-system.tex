...
\begin{itemize}
    \item \hqlabel{sec:walkthroughs:read-miss-n1}{\important{Read Miss from \texttt{N1}}}:
    \begin{enumerate}
    ...
    \item \hqlabel{sec:walkthroughs:write-miss-n2}{\important{Write Miss from \texttt{N2}}}:
    \begin{enumerate}
...
From here, we can consider the following scenarios:
\begin{itemize}
    \item \hqlabel{sec:walkthroughs:read-miss-n2}{\important{Read Miss from \texttt{N2}}}:
    \begin{enumerate}
...
From here, we can consider the following scenarios:
\begin{itemize}
    \item \hqlabel{sec:walkthroughs:read-miss-n2-from-modefied}{\important{Read Miss from \texttt{N2}}}:
    \begin{enumerate}
        \item \texttt{N2} (Local) sends a request (\texttt{ReadMiss}) to \texttt{N0} (Home) for block \texttt{B0}.
        \begin{gather*}
            \text{Local } (\texttt{N2}) \xrightarrow{\texttt{ReadMiss}} \text{Home } (\texttt{N0})
    ...
    \item \hqlabel{sec:walkthroughs:write-miss-n2-from-modified}{\important{Write Miss from \texttt{N2}}}:
    \begin{enumerate}
        \item \texttt{N2} (Local) sends a request (\texttt{WriteMiss}) to \texttt{N0} (Home) for block \texttt{B0}.
        \begin{gather*}
            \text{Local } (\texttt{N2}) \xrightarrow{\texttt{WriteMiss}} \text{Home } (\texttt{N0})
    ...
    \item \hqlabel{sec:walkthroughs:eviction-owner}{\important{Eviction at the Owner (Replacement)}}:
    \begin{enumerate}
