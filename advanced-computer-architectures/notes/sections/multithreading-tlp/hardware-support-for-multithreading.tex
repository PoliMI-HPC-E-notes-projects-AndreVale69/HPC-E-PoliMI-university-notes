...
\begin{itemize}
    \item \important{Fine-Grained MT} (\autopageref{def:fine-grain-thread}): can switch \textbf{every cycle}.
    \item \important{Coarse-Grained MT} (\autopageref{def:coarse-grain-thread}): switches on stalls $\rightarrow$ costs a few cycles for pipeline drain/refill, but much cheaper than OS context switches.
    \item \important{Simultaneous MT} (\autopageref{def:simultaneous-multithreading}): no ``switch'', but multiple threads issue in the same cycle, so contexts are interleaved continuously.
\end{itemize}
