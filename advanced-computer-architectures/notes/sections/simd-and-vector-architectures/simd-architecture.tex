...
\begin{itemize}
    ...
    \begin{itemize}
        \item \definition{Vector Register File (VRF)}: Large registers hold many elements (e.g., 64 floats).
        \item \definition{Vector Functional Units (VFUs)}: Operate in \emph{lanes} (each lane $=$ a PE). When we issue one vector instruction, the controller streams elements from the VRF through the VFUs.
