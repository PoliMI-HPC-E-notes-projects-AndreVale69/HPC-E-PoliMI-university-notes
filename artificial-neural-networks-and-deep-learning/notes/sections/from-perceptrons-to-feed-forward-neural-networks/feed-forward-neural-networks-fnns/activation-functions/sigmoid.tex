\paragraph{Sigmoid}\label{sec:sigmoid-activation-function}
...
\begin{itemize}
    ...
    \item The \textbf{orange shaded regions} highlight the parts of the sigmoid where the output is \textbf{almost constant}:
    \begin{itemize}
        \item On the left (for $a < -5$), the shaded area fills between the sigmoid and $1$, where the output is very close to $0$ (saturated low);
        \item On the right (for $a > 5$), the shaded area fills between the sigmoid and $0$, where the output is very close to $1$ (saturated high).
    \end{itemize}
    In those regions:
    \begin{equation*}
    ...
    \item The red dashed curve shows the \textbf{derivative} $f'(a)$ (plotted on the \textbf{right y-axis} with scale $0$ to $0.25$):
    \begin{equation*}
        f'(a) = f(a) \cdot (1 - f(a))
    \end{equation*}
    The derivative is only significant in a \textbf{small central region} (roughly between $-3$ and $3$), where it reaches a maximum of $0.25$ at $a=0$. Outside this range, the derivative \textbf{drops to near zero}, indicating that the neuron is \textbf{saturated} and \textbf{not learning effectively}. Notice how the derivative vanishes precisely in the orange saturation regions.
\end{itemize}
