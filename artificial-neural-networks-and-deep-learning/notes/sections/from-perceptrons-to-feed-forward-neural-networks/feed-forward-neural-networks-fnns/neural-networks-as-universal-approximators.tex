...
\begin{theorem}[Universal Approximation Theorem]\index{Universal Approximation Theorem}
    Let $K \subset \mathbb{R}^n$ be a compact set, and let:
    \begin{equation*}
        f : K \to \mathbb{R}
    \end{equation*}
    Be a continuous function. Then, for every $\varepsilon > 0$, there exist:
    \begin{itemize}
        \item a Feed-Forward \textbf{Neural Network with a single hidden layer},
        \item a \textbf{finite} number of \textbf{hidden neurons} $J$,
        \item weights $w_{ji}^{(1)}, w_j^{(2)} \in \mathbb{R}$,
        \item biases $b_j \in \mathbb{R}$,
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item $b_j$ are the \textbf{biases} of the \textbf{hidden layer} neurons.
        \item $\sum_{i=1}^{n} w_{ji}^{(1)} x_i + b_j$ is the \textbf{input to the hidden layer} neuron $j$.
        \item $w_j^{(2)}$ are the \textbf{weights from hidden layer to output layer}.
        \item $\sigma(\cdot)$ is the \textbf{non-linear activation function} applied at \textbf{hidden} layer neurons. For example, a common choice is the \textbf{sigmoid activation function} (\autopageref{sec:sigmoid-activation-function}):
        \begin{equation*}
            \sigma(a) = \dfrac{1}{1 + e^{-a}}
        \end{equation*}
        \item The output of the neural network is the weighted sum of the activated hidden neurons, which approximates $f(x)$ within $\varepsilon$ for all $x \in K$ (i.e., the neural network can get arbitrarily close to $f(x)$).
    \end{itemize}
\end{theorem}

\noindent
In simpler terms, any continuous function can be represented by a neural network with just \textbf{one hidden layer}, if that layer has enough neurons and uses a non-linear activation function.

\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{question-circle} \textbf{Why it works (intuition)}}
\end{flushleft}
Each hidden neuron with non-linear activation acts like a \textbf{basis function}. By combining enough of these basis functions (hidden neurons), the neural network can approximate complex functions by adjusting the weights and biases.\footnote{%
