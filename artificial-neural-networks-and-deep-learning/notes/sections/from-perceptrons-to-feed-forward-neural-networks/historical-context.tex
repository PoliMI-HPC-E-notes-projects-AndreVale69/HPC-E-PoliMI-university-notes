...
\begin{enumerate}
    \item \important{Weighted sum} of its inputs (integration): $\sum_{i=1}^{I} w_{i} x_{i}$.
    \item \important{Subtracts the bias} (thresholding): $\sum_{i=1}^{I} w_{i} x_{i} - b$.
    \item \important{Applies the activation function} (firing decision): $f\left(\sum_{i=1}^{I} w_{i} x_{i} - b\right)$.
\end{enumerate}
