...
\highspace
From this inspiration arose the idea of \textbf{neural networks}: mathematical models built from simple interconnected units that imitate, in a highly abstract way, the behavior of biological neurons. Interestingly, neural networks are not a recent invention of the deep learning era: they have existed since the birth of AI itself. In fact, the phrase ``\emph{Deep Learning is not AI, nor Machine Learning}'' is often used pedagogically to emphasize that \textbf{deep learning is only a specific family of techniques within machine learning, which itself is a subfield of artificial intelligence}. Neural networks therefore belong to a broader historical research tradition that predates modern deep learning.
...
\begin{itemize}
    \item \textbf{1940s motivation}: classic computers were excellent at precise and fast arithmetic, but researchers wanted systems that could \textbf{interact with noisy data}, operate in \textbf{parallel}, be \textbf{fault-tolerant}, and \textbf{adapt}.
...
\highspace
At this stage, AI and neural networks were almost inseparable: \textbf{building an intelligent machine was often seen as building an artificial brain}. In the following decades, this original vision split into two major traditions. The first was the \emph{symbolic} approach, based on explicit rules, logic, and hand-crafted representations. The second was the \emph{connectionist} approach, where systems learn from data through networks of simple computational units. Although the connectionist line remained in the background for many years, it later re-emerged as the foundation of what we now call \important{Deep Learning}.
...
\begin{figure}[!htp]
    \centering
    \input{img/biological-neuron-diagram.tikz}
    \caption{\textbf{Biological neuron structure}. The \emph{dendrites} receive \emph{excitatory} (green) and \emph{inhibitory} (red) signals through synapses, which are then integrated in the \emph{soma} (cell body). If the total accumulated signal exceeds a threshold, the neuron \emph{fires}, sending an output through the \emph{axon} to other neurons.}
...
Where:
\begin{itemize}
    \item $x_{i}$ are the \textbf{input values} (analogous to signals received by dendrites). They are like the neurotransmitter signals that a biological neuron receives from other neurons.
    \item $w_{i}$ are the \textbf{weights} (analogous to synaptic strengths). They represent how strongly each input influences the neuron's activation.
    \item $b$ is the \textbf{bias} (analogous to the threshold). It determines the level of input required for the neuron to activate.
    \item $f\left(\cdot\right)$ is the \textbf{activation function} (analogous to the firing mechanism). It decides whether the neuron fires based on the integrated input.
\end{itemize}
...
\begin{definitionbox}[: Artificial Neuron]\label{def:artificial-neuron}
    An \definition{Artificial Neuron} is a \textbf{mathematical model} inspired by the way a biological neuron works (\autopageref{fig:biological-neuron}). It's the \textbf{basic computation unit} of a neural network.
