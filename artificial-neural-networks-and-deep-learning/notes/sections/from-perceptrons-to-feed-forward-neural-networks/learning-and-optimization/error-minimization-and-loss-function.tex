...
In order to teach a neural network, we need a way to \textbf{measure how incorrect it is}. This measurement is called the \textbf{error (or loss) function}. Once defined, we can \textbf{minimize it} by adjusting the weights. This process is the essence of learning.
...
\begin{definitionbox}[: Error Function]
    ...
    \end{equation}
    Where $\text{Loss}\left(\cdot\right)$ is the per-sample difference between the predicted and actual value (i.e., the loss function). The \hl{error function aggregates these losses across the entire training set to give a single scalar value representing how well the model is performing overall.} The goal of learning is to find the weights $w$ that minimize this error function, thus improving the model's predictions (formally, \autoref{eq:optimal-weights} on \autopageref{eq:optimal-weights}).
\end{definitionbox}

\begin{definitionbox}[: Loss Function]\phantomsection\label{def:loss-function}
...
\begin{equation}\label{eq:sse-loss-function}
...
\highspace
Minimizing the SSE means finding the weights $w$ that make the network's predictions as close as possible to the true targets across the entire training set. Formally, \textbf{learning becomes finding the set of weights} $w^{*}$ \textbf{that minimize the total error}:
