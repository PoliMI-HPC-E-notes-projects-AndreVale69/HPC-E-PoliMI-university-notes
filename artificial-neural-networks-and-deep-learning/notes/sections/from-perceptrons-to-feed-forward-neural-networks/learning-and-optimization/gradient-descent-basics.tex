...
We've defined the learning goal (\autopageref{eq:optimal-weights}):
\begin{equation*}
    w^{*} = \arg\min_{w} E\left(w\right)
\end{equation*}
Where $E(w)$ is our \textbf{error (or loss) function}, for instance the Sum of Squared Errors (SSE, \autoref{eq:sse-loss-function} on \autopageref{eq:sse-loss-function}):
\begin{equation*}
...
\begin{definitionbox}[: Gradient Descent]\phantomsection\label{def:gradient-descent}
    ...
    \begin{equation}
        w_{k+1} = w_{k} - \eta \cdot \nabla E(w_{k})
    \end{equation}
    Where:
    \begin{itemize}
        \item $w_{k}$ is the weight vector at iteration $k$.
        \item $\nabla E(w_{k})$ is the gradient (slope) of the error function at $w_{k}$.
        \item $\eta$ (eta) is the \textbf{learning rate} (step size), a small positive scalar that controls the step size.
    ...
    \begin{itemize}
        ...
        \begin{center}
            \tikzsetnextfilename{gradient-descent-weight-notation}
    ...
    \begin{equation}\label{eq:gradient-neural-network}
        \nabla E(w) = \left\{
    ...
    \begin{equation}\label{eq:gradient-descent-neural-network-update}
...
\begin{enumerate}
    ...
    \begin{equation*}
        w_{ij}^{(l)} \leftarrow w_{ij}^{(l)} - \eta \cdot \dfrac{\partial \, E}{\partial \, w_{ij}^{(l)}}
        \quad , \quad
        b_{i}^{(l)} \leftarrow b_{i}^{(l)} - \eta \cdot \dfrac{\partial \, E}{\partial \, b_{i}^{(l)}}
    \end{equation*}
