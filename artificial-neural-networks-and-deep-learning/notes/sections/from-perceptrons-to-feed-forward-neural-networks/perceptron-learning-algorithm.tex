...
\begin{enumerate}
    ...
    However, this approach can be \hl{computationally expensive for large\break datasets since it requires summing over all misclassified points in each update}. Furthermore, there is \hl{no randomness}, which can lead to slow convergence and getting stuck in suboptimal solutions. To address these issues, we can add a pinch of \textbf{stochasticity} to the learning process.
