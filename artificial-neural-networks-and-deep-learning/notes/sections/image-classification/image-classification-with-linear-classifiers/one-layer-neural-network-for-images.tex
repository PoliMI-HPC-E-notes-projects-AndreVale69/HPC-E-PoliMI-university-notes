...
\highspace
\textcolor{Green3}{\faIcon{question-circle} \textbf{Why is Softmax ignored?}} Usually classification networks finish with a Softmax layer to convert the scores into probabilities. However, we can ignore it because \textbf{Softmax never changes the order of the scores}. Therefore, the class with the highest score before Softmax will also have the highest probability after Softmax. Softmax is \hl{useful during training} (to compute cross-entropy loss) or when calibrated probabilities are needed, but it is \textbf{not required to determine the predicted class}.
