...
\highspace
\textcolor{Red2}{\faIcon{exclamation-triangle} \textbf{The hidden assumption.}} Everything depends on the distance. The \textbf{algorithm assumes that a small pixel distance between two images implies that they are perceptually similar} (\autopageref{sec:perceptual-similarity-vs-pixel-similarity}). But this is not always true. The algorithm is only as good as the distance measure it uses. We will talk about this in the next section.