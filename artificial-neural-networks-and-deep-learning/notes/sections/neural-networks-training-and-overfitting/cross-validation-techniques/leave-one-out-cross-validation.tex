...
Input:
    // Dataset composed of $\textcolor{codegray}{N}$ samples,
...
for $i = 1$ to $N$:
    // Create validation set using the $\textcolor{codegray}{i}$-th sample
    $\mathcal{D}_{\text{val}} \leftarrow \{(x_i,t_i)\}$
    // Create training set by excluding the $\textcolor{codegray}{i}$-th sample
    $\mathcal{D}_{\text{train}} \leftarrow \mathcal{D} \setminus \{(x_i,t_i)\}$

    Train the model:
        // Train a model $\textcolor{codegray}{f^{(i)}}$ using the learning algorithm $\textcolor{codegray}{\mathcal{A}}$
        // on the training set $\textcolor{codegray}{\mathcal{D}_{\text{train}}}$
        $f^{(i)} \leftarrow \mathcal{A}(\mathcal{D}_{\text{train}})$

    Compute prediction on validation sample:
        // Predict the output for the validation sample $\textcolor{codegray}{x_i}$
        // using the trained model $\textcolor{codegray}{f^{(i)}}$
        $\hat{t}_i \leftarrow f^{(i)}(x_i)$

    Compute validation error:
        // Compute the error $\textcolor{codegray}{E_i}$ for the validation sample
        // using the error function $\textcolor{codegray}{\mathcal{L}}$
