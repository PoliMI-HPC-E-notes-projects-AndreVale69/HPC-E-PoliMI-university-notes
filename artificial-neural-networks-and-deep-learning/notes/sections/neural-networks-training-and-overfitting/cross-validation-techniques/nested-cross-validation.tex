...
// For each outer fold
for $k = 1$ to $K$:
    // Create outer test set using the $\textcolor{codegray}{k}$-th fold
    ...
    for each configuration $\theta \in \Theta$:
        ...
        // For each inner fold
        for $m = 1$ to $M$:
            // Create inner validation set
            // using the $\textcolor{codegray}{m}$-th fold
            $\mathcal{D}_{\text{val}}^{(m)} \leftarrow \mathcal{D}_m^{\text{inner}}$
            // Create inner training set
            // using the remaining folds
            $\mathcal{D}_{\text{train}}^{(m)} \leftarrow
            \mathcal{D}_{\text{train}}^{(k)} \setminus \mathcal{D}_m^{\text{inner}}$

            Train inner model:
                // Train model $\textcolor{codegray}{f_{\theta}^{(m)}}$ on $\textcolor{codegray}{\mathcal{D}_{\text{train}}^{(m)}}$
                // with configuration $\textcolor{codegray}{\theta}$
                $f_{\theta}^{(m)} \leftarrow \mathcal{A}(\mathcal{D}_{\text{train}}^{(m)}, \theta)$

            Compute inner validation error:
                // Compute error $\textcolor{codegray}{e_m}$ on $\textcolor{codegray}{\mathcal{D}_{\text{val}}^{(m)}}$
                // using model $\textcolor{codegray}{f_{\theta}^{(m)}}$
