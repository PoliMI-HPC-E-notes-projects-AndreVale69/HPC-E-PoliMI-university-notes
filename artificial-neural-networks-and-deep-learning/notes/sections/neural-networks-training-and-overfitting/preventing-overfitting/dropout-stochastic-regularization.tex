...
\begin{equation}
    \tilde{h}_j^{(l)} = m_j^{(l)} \cdot h_j^{(l)} = m_j^{(l)} \cdot g\left( \sum_{i} w_{ij}^{(l)} h_i^{(l-1)} + b_j^{(l)} \right)
\end{equation}
\begin{itemize}
...
\begin{equation*}
    \tilde{h}_j^{(l)} = m_j^{(l)} \cdot h_j^{(l)}
\end{equation*}
During \textbf{testing}, dropout is disabled, so we use the \textbf{full network}, meaning that all neurons are active. However, during training each neuron was active only with probability $p$, so its \textbf{expected contribution} was smaller. To keep the scale of activations consistent between training and testing, we multiply each activation (equivalently, the outgoing weights) by $p$:
\begin{equation*}
    h_j^{(l,\text{test})} = p \cdot h_j^{(l)}
\end{equation*}
This is called the \definition{Weight Scaling Rule}.

\highspace
\begin{definitionbox}[: Weight Scaling Rule]
    In the \textbf{classical dropout formulation}, during testing we do \textbf{not} drop any neuron. Instead, we use the full network and \textbf{scale each activation by the keep probability} $p$:
    \begin{equation}
        h_j^{(l,\text{test})} = p \cdot h_j^{(l)}
    \end{equation}
    ...
    \highspace
    This is called the \definition{Weight Scaling Rule}. Its purpose is to keep the \textbf{expected activation at test time equal to the expected activation during training}. Indeed, during training:
    \begin{equation}
        \tilde{h}_j^{(l)} = m_j^{(l)} \cdot h_j^{(l)},
        \qquad m_j^{(l)} \sim \text{Bernoulli}(p)
    \end{equation}
    so:
    \begin{equation}
        \mathbb{E}\left[\tilde{h}_j^{(l)}\right]
        =
        \mathbb{E}\left[m_j^{(l)}\right] \cdot h_j^{(l)}
        =
        p \cdot h_j^{(l)}
    \end{equation}
...
\begin{remarkbox}[: Inverted Dropout in Practice]
    Modern deep learning libraries usually implement \textbf{inverted dropout} instead of the classical weight scaling rule. In inverted dropout:
    \begin{itemize}
        \item During training, kept activations are scaled by $\frac{1}{p}$:
        \begin{equation*}
            \tilde{h}_j^{(l)} = \dfrac{m_j^{(l)}}{p} \cdot h_j^{(l)}
        \end{equation*}
        \item During testing, no scaling is needed:
        \begin{equation*}
