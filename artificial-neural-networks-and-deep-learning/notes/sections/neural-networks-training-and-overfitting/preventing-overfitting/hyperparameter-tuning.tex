...
// (Optional) It is optional because the model $\textcolor{codegray}{f^*}$ trained 
// with $\textcolor{codegray}{\theta^*}$ on $\textcolor{codegray}{\mathcal{D}_{\text{train}}}$ may already be good enough.
// However, if we want to maximize the data used for
// training, we can retrain the model with the
...
Input:
    Training dataset $\mathcal{D}_{\text{train}}$
    Validation dataset $\mathcal{D}_{\text{val}}$
    Hyperparameter distributions $P(\theta)$ // e.g. log-uniform
    Number of trials $N$
    Learning algorithm $\mathcal{A}$
    Validation error function $E_{\text{val}}(\cdot)$
...
// The loop starts from $\textcolor{codegray}{N_{\text{init}}+1}$ because we have already
// evaluated $\textcolor{codegray}{N_{\text{init}}}$ random configurations.
// We continue until we reach the maximum number
// of iter. $\textcolor{codegray}{T}$
for $t = N_{\text{init}}+1$ to $T$:
    // Create a (surrogate) model
    // from the observations $\textcolor{codegray}{\mathcal{H}}$,
    // which is a set of pairs
    // (configuration, validation error)
    Fit surrogate model on $\mathcal{H}$

    // The acquisition function $\textcolor{codegray}{a(\theta \mid \mathcal{H})}$ uses the surrogate
    // model to estimate the potential improvement of
    // evaluating a new configuration $\textcolor{codegray}{\theta}$ based
    // on the observed data $\textcolor{codegray}{\mathcal{H}}$. It balances:
    //  1. exploration (trying configurations with high
    //     uncertainty)
    //  2. exploitation (focusing on configurations that
    //     are likely to perform well).
    Define acquisition function $a(\theta \mid \mathcal{H})$

    Choose next configuration:
        // Choose the configuration $\textcolor{codegray}{\theta_t}$ that maximizes
    ...
    Update observation set:
        // Add the new configuration and its validation
        // error to the observation set $\textcolor{codegray}{\mathcal{H}}$,
        // which will be used to update the surrogate
...
Select best observed configuration:
    ...
    // we select the configuration with the lowest
    // validation error from the observation set $\textcolor{codegray}{\mathcal{H}}$.
