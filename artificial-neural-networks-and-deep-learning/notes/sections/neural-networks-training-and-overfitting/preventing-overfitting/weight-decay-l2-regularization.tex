...
\begin{equation*}
    E_{\text{train}}(w) = \frac{1}{N} \cdot \sum_{i=1}^{N} \mathcal{L}\left(y_i, f\left(x_i; w\right)\right)
\end{equation*}
...
\begin{equation*}
    E_{\text{reg}}(w) = E_{\text{train}}(w) + \underbrace{\frac{\gamma}{2} \cdot \sum_{q} w_q^2}_{\text{L2 penalty}}
\end{equation*}
\begin{itemize}
...
\begin{definitionbox}[: Weight Decay (L2 Regularization)]
    ...
    \begin{equation}
        E_{\text{reg}}(w) = E_{\text{train}}(w) + \frac{\gamma}{2} \cdot \sum_{q} w_q^2
    \end{equation}
    Where:
    \begin{itemize}
...
\end{flushleft}
Early Stopping limited training time to prevent overfitting (\autopageref{sec:early-stopping}), while \textbf{weight decay directly limits the magnitude of parameters}. Both methods implement the Ockham's Razor principle (page \hqpageref{sec:ockhams-razor}) by \textbf{favoring simpler models that generalize better}, but act at different stages:
\begin{itemize}
