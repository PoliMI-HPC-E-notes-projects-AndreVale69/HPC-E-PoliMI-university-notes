...
\begin{itemize}
    ...
    \item \important{Golden Rule}: Always keep the test set completely separate until the very end. Use training and validation sets for model development, and only use the test set for final evaluation. So, \hl{never use validation/test data to update model weights.} Otherwise, we risk \textbf{data leakage}.
\end{itemize}

\highspace
\begin{definitionbox}[: Data Leakage]
    \definition{Data Leakage} occurs when \textbf{information from outside the training dataset is used to create the model}, leading to overly optimistic performance estimates. This can happen if:
    \begin{itemize}
        \item The test set is accidentally used during training or hyperparameter tuning.
        \item Features that won't be available in real-world scenarios are included in the training data.
        \item The same data points are present in both training and test sets.
    \end{itemize}
    \textcolor{Red2}{\faIcon{exclamation-triangle}} Data leakage can cause a model to perform well on the test set but fail in real-world applications, as it has effectively ``cheated'' by learning from information it shouldn't have access to.
\end{definitionbox}

\begin{figure}[!htp]
    \centering
    \tikzsetnextfilename{terminology-clarifications-dataset-split}
    \begin{tikzpicture}[
        font=\sffamily\small,
        >=Latex,
        node distance=10mm,
