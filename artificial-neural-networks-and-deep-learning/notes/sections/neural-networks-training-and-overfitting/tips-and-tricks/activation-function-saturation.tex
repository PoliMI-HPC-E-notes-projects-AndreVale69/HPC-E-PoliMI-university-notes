...
\end{itemize}
Now, both are \textbf{S-shaped} (see \autoref{fig:sigmoid-activation-function}, \autopageref{fig:sigmoid-activation-function}), they flatten for large positive or negative $z$ values:
...
\begin{equation*}
    w_{ij}^{(l)} \leftarrow w_{ij}^{(l)} - \eta \cdot \dfrac{\partial \, E}{\partial \, w_{ij}^{(l)}} \quad \text{and} \quad
...
\begin{equation*}
    \delta_{j}^{(l-1)} = g'(z_{j}^{(l-1)}) \cdot \sum_{i} w_{ij}^{(l)} \delta_{i}^{(l)}
\end{equation*}
