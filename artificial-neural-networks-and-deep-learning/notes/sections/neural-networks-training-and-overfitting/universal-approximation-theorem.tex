...
\begin{theorem}[\definition{Universal Approximation Theorem}]\label{thm:universal-approximation-theorem}
    ...
    \begin{itemize}
        ...
        \item $\alpha_{j}, w_{j}, b_{j}$ are the network parameters ($\alpha_{j}$ are the \textbf{output layer weights}, $w_{j}$ are the \textbf{input weights for the hidden layer}, and $b_{j}$ are the \textbf{biases}).
...
\begin{remarkbox}[: What is Overfitting?]\label{rmk:overfitting}
...
\begin{remarkbox}[: What is Underfitting?]\label{rmk:underfitting}
