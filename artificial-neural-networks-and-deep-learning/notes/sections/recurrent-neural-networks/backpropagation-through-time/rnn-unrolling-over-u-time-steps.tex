...
\definition{Backpropagation Through Time (BPTT)} \textbf{is the algorithm used to train Recurrent Neural Networks}. It is simply \textbf{backpropagation}\break (\autopageref{sec:backpropagation-conceptual-introduction}) \textbf{applied to the unrolled RNN}. However, an RNN is \textbf{not a standard feed-forward network} because it has \textbf{cycles} due to its recurrent connections:
\begin{equation*}
