...
\centering
\resizetikz{\textwidth}{!}{%
\tikzsetnextfilename{rnn-computation-graph-forward-backward}
\begin{tikzpicture}[
    >=Stealth,
...
\caption{\textbf{Zoom-in of a single RNN cell}. The cell combines the current input $x_t$ and the previous hidden state $h_{t-1}$ through two linear transformations, adds the bias $b$, and applies a nonlinearity $f(\cdot)$ to produce the new hidden state $h_t$.}
...
\begin{figure}[!htp]
    \centering
    \tikzsetnextfilename{rnn-unrolling}
    \begin{tikzpicture}[
        >=Stealth,
        node distance=1.6cm,
        every node/.style={font=\small},
