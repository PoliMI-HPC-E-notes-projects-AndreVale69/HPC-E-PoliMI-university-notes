\subsubsection{Feed-Forward Extensions: TDNNs}\label{sec:feed-forward-extensions-time-delay-neural-networks-tdnn}
...
\begin{equation*}
    \mathbf{x}_t = \left[x_{t-1}, x_{t-2}, \ldots, x_{t-p}\right]^T
\end{equation*}
...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{balance-scale} \textbf{Autoregressive vs TDNN}}
\end{flushleft}
Both AR models and TDNNs are \textbf{memoryless} in the sense that they \textbf{do not maintain an internal state} across time steps. Therefore, \textbf{we manually provide past values as input}. Both models use past values, but in \textbf{different ways}:
\begin{itemize}
    \item \important{AR Models} predict the next value using \textbf{past outputs}:
    \begin{equation*}
        y_t = f\left(y_{t-1}, y_{t-2}, \dots, y_{t-p}\right)
    \end{equation*}


    \item \important{TDNNs} predict the next value using \textbf{past inputs}:
    \begin{equation*}
        y_t = f\left(x_{t-1}, x_{t-2}, \dots, x_{t-p}\right)
    \end{equation*}
\end{itemize}
Autoregressive and time-delay neural networks \textbf{differ in the type of inputs they use} (past outputs vs past inputs), not in whether they are linear or non linear; both can be linear or nonlinear depending on the function $f$ used.

\highspace
Neither Autoregressive nor TDNN is better or worse; they are simply \textbf{different approaches to modeling temporal data}. AR models use past predictions as input, which can lead to error accumulation if the predictions are inaccurate. TDNNs use past observed values, which tend to be more stable, though they may require more complex architectures to capture long-term dependencies. Which approach is better depends on the specific problem, the characteristics of the data, and the available computational resources.