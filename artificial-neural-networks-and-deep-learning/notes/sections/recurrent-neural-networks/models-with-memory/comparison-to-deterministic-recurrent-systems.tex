...
\begin{itemize}
    ...
    \textcolor{Green3}{\faIcon{check} \textbf{Key Properties}}
    \begin{itemize}
        \item \textbf{No noise} terms.
        \item \textbf{No probability} distributions.
        \item \textbf{Hidden state} evolves \textbf{deterministically}.
        \item \textbf{Same input sequence}, then same hidden state trajectory.
        \item \textbf{Learning} (if any) is achieved via optimization of a loss function (e.g., MSE, cross-entropy) \textbf{using gradient-based methods}, \underline{not} via probabilistic inference.
    \end{itemize}
