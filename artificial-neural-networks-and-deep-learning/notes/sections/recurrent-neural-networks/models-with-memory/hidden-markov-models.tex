...
\begin{enumerate}
    ...
    \end{equation*}
    These states represent what is \emph{really happening} in the system, \underline{but} we \textbf{cannot observe them directly} (state estimation problem, \autopageref{def:state-estimation-problem}). Instead, they are \textbf{latent variables} that we need to infer from the observations. This layer evolves according to the \textbf{Markov Chain}:
    \begin{equation*}
    ...
    \begin{equation*}
        \textcolor{Green3}{\text{\faIcon{check-circle} } \textbf{Markov Assumption}} \, \Rightarrow \, P\left(s_t \, \mid \, s_{1}, s_{2}, \ldots, s_{t-1}\right) = P\left(s_t \, \mid \, s_{t-1}\right)
    \end{equation*}
    This means that the \textbf{next hidden state} depends \textbf{only on the current one}, not on the entire history of past states. This assumption \hl{simplifies the model and makes computations tractable}, because the entire history before $s_{t-1}$ is irrelevant once current state $s_{t-1}$ is known. \hl{Without this assumption}, the model would become significantly more complex and harder to work with (\textbf{exponentially many parameters}) because we would need to consider all previous states to predict the next one:
    \begin{equation*}
        \textcolor{Red2}{\text{\faIcon{exclamation-triangle} } \textbf{Without Markov Assumption}} \, \Rightarrow \, P\left(s_t \, \mid \, s_{1}, s_{2}, \ldots, s_{t-1}\right)
    \end{equation*}
    ...
    \begin{equation*}
        \textcolor{Green3}{\text{\faIcon{check-circle} } \textbf{Emission Independence}} \, \Rightarrow \, P\left(y_t \, \mid \, s_{t}, s_{t-1}, \dots, y_{t-1}\right) = P\left(y_t \, \mid \, s_t\right)
    \end{equation*}
    ...
    \begin{equation*}
        \textcolor{Red2}{\text{\faIcon{exclamation-triangle} } \textbf{Without Emission Independence}} \, \Rightarrow \, P\left(y_t \, \mid \, s_{t}, s_{t-1}, \dots, y_{t-1}\right)
    \end{equation*}
    Instead, the emission independence assumption allows us to \textbf{simplify the model and \hl{focus on the relationship between the current hidden state and the current observation}}:
    \begin{equation*}
        P\left(y_t \, \mid \, s_t\right) = b_{s_t}(y_t)
    \end{equation*}
\end{enumerate}
...
\end{flushleft}
Because without these assumptions, the probability of a sequence would involve \textbf{all states and all observations interacting} with each other directly. That would make modeling impossible, inference impossible and learning impossible (because the number of parameters would explode combinatorially). Because \textbf{hiddent transitions} and \textbf{observations} are two independent kinds of dependencies, we can \textbf{factor the joint probability} of the entire sequence into manageable parts. Given the two assumptions, the \hl{full joint probability of the hidden states and observations} can be expressed as:
\begin{equation}
    P\left(S, Y\right) = P\left(s_1\right) \cdot \prod_{t=2}^{T} P\left(s_t \, \mid \, s_{t-1}\right) \cdot \prod_{t=1}^{T} P\left(y_t \, \mid \, s_t\right)
\end{equation}
\begin{itemize}
    \item The \textbf{first term} $P\left(s_1\right)$ represents the \hl{probability of starting in the initial hidden state}.
    \item The \textbf{first product} multiplies all the state transition probabilities together, capturing \hl{how the hidden states evolve over time} according to the Markov chain.
    \item The \textbf{second product} multiplies all the observation emission probabilities together, capturing \hl{how each observation is generated from the corresponding hidden state}.
\end{itemize}
Obviously, this factorization would be \textbf{impossible} without the two assumptions (Emission Independence and Markov Property), because we would have to consider all possible dependencies between states and observations across time steps.
...
\end{flushleft}
Hidden Markov Models are powerful, but have one fundamental issue: \textbf{we don't see the hidden states, but everything depends on them}. When we have an HMM, we \textbf{don't directly observe the states}, but we only see:
\begin{equation*}
    Y = \left(y_1, y_2, \ldots, y_T\right)
\end{equation*}
These are the \textbf{observations} generated by the hidden states. But what we really want to know is the \textbf{hidden state sequence}:
\begin{equation*}
    S = \left(s_1, s_2, \ldots, s_T\right)
\end{equation*}
...
\begin{enumerate}
    \item \important{Evaluation (Likelihood) Problem}. ``\emph{How likely is this observed sequence under our model?}''. Given an HMM $\lambda = (\pi, A, B)$ and an observation sequence $Y = (y_1, y_2, \ldots, y_T)$, compute the probability of the observation sequence:
    \begin{equation*}
        P\left(Y \, \mid \, \lambda\right)
    \end{equation*}
    \textcolor{Red2}{\faIcon{exclamation-triangle} \textbf{Why is this hard?}} Because:
    \begin{equation*}
        P\left(Y \, \mid \, \lambda\right) = \sum_{S} P\left(S, Y \, \mid \, \lambda\right)
    \end{equation*}
    We need to sum over \textbf{all possible hidden state sequences} $S$, and there are \textbf{exponentially many} of them:
    \begin{equation*}
        K^{T} \quad \text{(if there are $K$ hidden states and $T$ time steps)}
    \end{equation*}
    ...
    \item \important{Decoding (Inference) Problem}. ``\emph{What is the most likely hidden state sequence?}''. Given an HMM $\lambda = (\pi, A, B)$ and an observation sequence $Y = (y_1, y_2, \ldots, y_T)$, find the most likely sequence of hidden states $S = (s_1, s_2, \ldots, s_T)$ that could have generated the observations:
    \begin{equation*}
        \hat{S} = \arg\max_{S} P\left(S \, \mid \, Y, \lambda\right)
    \end{equation*}
    \textcolor{Red2}{\faIcon{exclamation-triangle} \textbf{Why is this hard?}} \hl{Similar to the evaluation problem}, we would need to consider all possible hidden state sequences to find the one that maximizes the probability. This is also computationally infeasible due to the exponential number of sequences.
    ...
    \item \important{Learning (Training) Problem}. ``\emph{What parameters $\lambda = \left(\pi, \, A, \, B\right)$ best explain our observed data?}''.
    
    Given an observation sequence $Y = (y_1, y_2, \ldots, y_T)$, estimate the HMM parameters $\lambda = (\pi, A, B)$ that best explain the observed data. This involves finding the model parameters that maximize the likelihood of the observations:
    \begin{equation*}
        \lambda^* = \arg\max_{\lambda} P\left(Y \, \mid \, \lambda\right)
    \end{equation*}
    \textcolor{Red2}{\faIcon{exclamation-triangle} \textbf{Why is this hard?}} Because we do not observe the hidden states, we cannot directly count how many times each transition or emission occurs. We need to infer the hidden states while also estimating the parameters, which creates a circular dependency. This is a classic case of \textbf{unsupervised learning} where we have to learn from data without direct labels (the hidden states are like labels that we don't have).
...
\end{enumerate}
All 3 problems come from the same root: \hl{hidden variables make everything combinatorially complex}.
...
\begin{enumerate}
    ...
    \begin{equation*}
        s_t^* = \psi_{s_{t+1}^*}(t+1) \quad \text{for } t = T-1, T-2, \ldots, 1
    \end{equation*}
    At time $t+1$, we look at the best state $s_{t+1}^*$ and use $\psi$ to find which state at time $t$ led to it. We repeat this process until we reach the beginning of the sequence, reconstructing the entire most likely hidden state sequence $S^* = (s_1^*, s_2^*, \ldots, s_T^*)$.
\end{enumerate}

\begin{lstlisting}[language=pseudo-code,mathescape=true,style={pseudocode-style},caption={Viterbi Algorithm for HMM Decoding}]
Input:
    Observation sequence $Y = (y_1, y_2, \dots, y_T)$
    HMM parameters $\lambda = (\pi, A, B)$
    Number of hidden states $K$

Output:
    Most likely hidden state sequence $\hat{S} = (s_1^*, s_2^*, \dots, s_T^*)$

// Initialization
// For each state $\textcolor{codegray}{j}$, compute the initial Viterbi score
// (i.e., the probability of starting in state $\textcolor{codegray}{j}$ and emitting
// $\textcolor{codegray}{y_1}$) and set the pointer to 0
for $j = 1$ to $K$:
    $\delta_j(1) \leftarrow \pi_j \cdot b_j(y_1)$
    $\psi_j(1) \leftarrow 0$

// Recursion
// For each time step from 2 to $\textcolor{codegray}{T}$,
// compute the Viterbi scores and pointers for each state $\textcolor{codegray}{j}$
// by considering all possible previous states $\textcolor{codegray}{i}$
// and taking the maximum over them
for $t = 2$ to $T$:
    for $j = 1$ to $K$:
        $\delta_j(t) \leftarrow \max_{i \in \{1,\dots,K\}}
            \left[ \delta_i(t-1) \cdot a_{ij} \right] \cdot b_j(y_t)$
        $\psi_j(t) \leftarrow \arg\max_{i \in \{1,\dots,K\}}
            \left[ \delta_i(t-1) \cdot a_{ij} \right]$

// Termination
// Find the best score and the final state of the best path
$P^* \leftarrow \max_{j \in \{1,\dots,K\}} \delta_j(T)$
$s_T^* \leftarrow \arg\max_{j \in \{1,\dots,K\}} \delta_j(T)$

// Backtracking
// Reconstruct the most likely hidden state sequence
// by backtracking through the pointers
for $t = T-1$ down to $1$:
    $s_t^* \leftarrow \psi_{s_{t+1}^*}(t+1)$

Return $\hat{S} = (s_1^*, s_2^*, \dots, s_T^*)$
\end{lstlisting}
