...
\begin{examplebox}[: Linear Dynamical System (LDS)]
    ...
    \end{itemize}
    If the hidden state $\mathbf{h}_t$ can't be observed directly, we can estimate it using the \textbf{Kalman filter}.
\end{examplebox}
...
\begin{definitionbox}[: Markov Property]
    ...
    \end{equation}
    So, once we know th \textbf{current state} $h_{t-1}$, the \textbf{past states} $h_{t-2}, \ldots, h_0$ provide no additional information about the \textbf{future state} $h_t$. All relevant information from the past is \textbf{summarized} inside the current state $h_{t-1}$.
\end{definitionbox}
