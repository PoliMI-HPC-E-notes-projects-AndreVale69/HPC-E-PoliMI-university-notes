...
\begin{equation}\label{eq:sequence-modeling-probability}
    P\left(\mathbf{x}_t \mid \mathbf{x}_{t-1}, \mathbf{x}_{t-2}, \dots, \mathbf{x}_0\right)
\end{equation}
This \hl{conditional probability} captures the idea that the \hl{current observation $\mathbf{x}_t$ depends on all previous observations in the sequence}.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Autoregressive (AR) Models} (\autopageref{sec:autoregressive-models}): Predict the current output based on a fixed number of previous inputs.
        \item \textbf{Feed-Forward Time-Delay Neural Network (TDNN)} (page \pageref{sec:feed-forward-extensions-time-delay-neural-networks-tdnn}): Use a sliding window of past inputs as additional features.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Linear Dynamical Systems (LDS)} (\autopageref{sec:lds-kalman-filter}): Use linear transformations to update the hidden state over time.
        \item \textbf{Hidden Markov Models (HMMs)} (\autopageref{sec:hidden-markov-models}): Use probabilistic transitions between hidden states to model sequences.
        \item \textbf{Recurrent Neural Networks (RNNs)} (\autopageref{sec:recurrent-neural-networks}): Use non-linear functions to update the hidden state, allowing for complex temporal dependencies.
    \end{itemize}
\end{itemize}