...
\end{equation*}
\begin{enumerate}
    \item Utilizzando gli opportuni comandi Matlab, si costruiscano la matrice $A$ ed il vettore $\mathbf{b}$ per $n = 7$. Utilizzando il comando \texttt{\textbackslash} si risolva il sistema lineare $A\mathbf{x} = \mathbf{b}$. Riportare $\mathbf{x}$ e tutti i comandi Matlab usati.
...
condition = true;
for row = 1:length(A)
    % valore sulla diagonale
    a_ii = abs(A(row, row));
    % somma della riga inclusa la diagonale
    row_sum = sum(abs(A(row, 1:n)));
    % se la condizione per la row-esima
    % non e' valida, exit
    if ~(a_ii > (row_sum - a_ii))
        condition = false;
        break
    end
end
if condition
    disp("La matrice A e' a dominanza diagonale stretta per righe");
else
    disp("La matrice A non e' a dominanza diagonale stretta per righe");
end
...
if first_m_cond && second_m_cond && third_m_cond
    ...
    Output:
    \begin{lstlisting}
La matrice A non e' a dominanza diagonale stretta per righe
La matrice e' simmetrica definita positiva
La matrice e' una M-matrice non singolare\end{lstlisting}
    Quindi, la matrice $A$ soddisfa le condizioni di essere simmetrica definita positiva e di essere una M-matrice non singolare, ma non soddisfa la condizione di dominanza diagonale stretta per righe.
    Questo comporta che il metodo di Gauss-Seidel converge comunque, in quanto la condizione di dominanza diagonale stretta per righe è solo una condizione sufficiente, ma non necessaria. Infatti, la condizione necessaria e sufficiente per la convergenza del metodo di Gauss-Seidel è che il raggio spettrale della matrice di iterazione $B_{GS}$ sia minore di uno, condizione che è soddisfatta in questo caso:
    \begin{lstlisting}[language=MATLAB]
B_gs = eye(size(A,1)) - tril(A)\A;
rho_gs = max(abs(eig(B_gs)));
disp("Raggio spettrale di B_gs: " + rho_gs);
disp("Il metodo Gauss-Seidel converge? " + (rho_gs < 1));\end{lstlisting}
    Output:
    \begin{lstlisting}
Raggio spettrale di B_gs: 0.85355
Il metodo Gauss-Seidel converge? true\end{lstlisting}
...
\begin{enumerate}
    ...
    \end{enumerate}
    A pagina \hqpageref{problema-cauchy-con-eulero-indietro} si trova un riepilogo del metodo di Eulero all'indietro con il metodo di Newton.
