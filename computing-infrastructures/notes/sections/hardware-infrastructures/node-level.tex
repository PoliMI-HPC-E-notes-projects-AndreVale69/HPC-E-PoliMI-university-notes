...
\begin{enumerate}
    ...
    \item \label{def: SCAN - Elevator Algorithm} \definition{SCAN (Elevator Algorithm)} (good performance as SSTF, but fairer)
    \begin{itemize}
    ...
    \item \label{def: C-SCAN - Elevator Algorithm} \definition{Circular SCAN (C-SCAN)} (fair, but less efficient than SCAN in some cases)
    \begin{itemize}
...
\begin{itemize}
    \item \hqlabel{RAID 0 + 1}{\definition{RAID 0 $+$ 1}} is a \textbf{striped array of mirrored sets}. It combines the performance of RAID 0 and the fault tolerance of RAID 1, but with \textbf{some limitations}.
    ...
    \item \hqlabel{RAID 1 + 0}{\definition{RAID 1 $+$ 0}} combines \textbf{mirroring (RAID 1)} and \textbf{striping (RAID 0)}. First, data is \textbf{mirrored}, then it is \textbf{striped} across mirrored pairs. It offers \textbf{high performance (like RAID 0)}, \textbf{high fault tolerance (like RAID 1)}.
...
\highspace
\begin{flushleft}
    \hqlabel{def: The Consistent Update Problem}{\textcolor{Red2}{\faIcon{exclamation-triangle} \textbf{The Consistent Update Problem}}}
\end{flushleft}
...
\begin{flushleft}
    \textcolor{Green3}{\faIcon{shield-alt} \textbf{Mean Time To Failure (MTTF) of RAID 5}}
\end{flushleft}
...
\begin{enumerate}
    \item \important{Wait for the \emph{first} disk to fail}. We have $G$ disks. As we have seen before, the failure rate of each disk is:
    \begin{equation*}
    ...
    \end{equation*}
    \item \important{Risk of a second disk failing while you rebuild}. When one disk fails, we have $G-1$ disks left. During the \textbf{vulnerable window} (MTTR), each remaining disk could fail. As we saw on page \pageref{eq: RAID 5 - probability that a disk fails in a small time}, the chance that one disk will fail within that window is:
    \begin{equation*}
        P(\text{fail in MTTR}) = \dfrac{\text{MTTR}}{\text{MTTF}}
    \end{equation*}
    ...
    \end{equation*}
\end{enumerate}
We now have everything we need to calculate the \definitionWithSpecificIndex{MTTF of the RAID 5}{RAID 5 - MTTF}{}:
...
\end{center}
\definition{RAID 6} is an extension of RAID 5 that adds \textbf{a second independent parity block} per stripe. This allows it to \textbf{tolerate two simultaneous disk failures}, a major reliability upgrade over RAID 5.
...
\begin{itemize}
    ...
    \item \textbf{Q parity}: is more complex. It is generated using \definition{Reed-Solomon} coding, a mathematical technique over \definition{Galois Fields (GF)}, which enables the correction of multiple simultaneous failures.
\end{itemize}
...
\highspace
\begin{flushleft}
    \textcolor{Red2}{\faIcon{exclamation-triangle} \textbf{Write Performance}}
\end{flushleft}
...
\begin{itemize}
    \item \definition{Hot Spare Disk}. A \emph{hot spare} is a \textbf{physical disk} already installed and powered in the system but \textbf{not actively used} to store data or parity. It's \textbf{just waiting}.
    ...
    \begin{itemize}
        \item \definition{Hardware RAID}. Implemented via a \textbf{dedicated RAID controller card} or built into the motherboard.
        \begin{itemize}
            \item[\textcolor{Green3}{\faIcon{check-circle}}] \textcolor{Green3}{\textbf{Pros}}
            \begin{itemize}[label=\textcolor{Green3}{\faIcon{check}}]
        ...
        \item \definition{Software RAID}. Managed by the \textbf{operating system} (e.g., Linux \texttt{mdadm}, Windows Storage Spaces).
        \begin{itemize}
            \item[\textcolor{Green3}{\faIcon{check-circle}}] \textcolor{Green3}{\textbf{Pros}}
            \begin{itemize}[label=\textcolor{Green3}{\faIcon{check}}]
            ...
            \begin{itemize}[label=\textcolor{Red2}{\faIcon{times}}]
                ...
                \item Vulnerable to the \textbf{consistent update problem} (page \hqpageref{def: The Consistent Update Problem}).
            \end{itemize}
        \end{itemize}
    \end{itemize}
\end{itemize}
...
\begin{examplebox}[: Bisection Bandwidth]
    Suppose:
    \begin{itemize}
...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{question-circle} \textbf{How to design a data center network? Design principles}}
\end{flushleft}
...
\paragraph{Switch-centric: classical Three-Tier architecture}\label{paragraph: classical Three-Tier architecture}
...
\begin{itemize}
    \item \definition{ToR (Top-of-Rack) architecture}. It is a \textbf{switching architecture} for connecting servers \textbf{inside a rack} to the rest of the datacenter network. The idea is simple, we place a \textbf{network switch at the top of each rack}, this is the \definition{Top-of-Rack switch (ToR switch)}. All servers inside that rack connect \textbf{upwards} to that switch. The ToR switch then connects \textbf{upstream} to aggregation/core switches that connect the whole datacenter.
    ...
    \item \definition{EoR (End-of-Row) architecture}. It is a classic \textbf{datacenter network architecture} for connecting racks of servers.
    ...
    \textcolor{Green3}{\faIcon{question-circle} \textbf{How does it work physically?}} Imagine a row of 10 racks. Each rack has no ToR switch, instead:
    \begin{itemize}
...
The \definition{Leaf-Spine Architecture} is a \textbf{two-tier network topology} used in modern data centers to \textbf{handle massive East-West traffic} and \hl{maximize bisection bandwidth}.
\begin{itemize}
    \item \definition{Leaf switches} are the ``\textbf{bottom layer}''. These are basically \textbf{ToR (Top-of-Rack)} switches, they connect directly to the servers in each rack.
    \item \definition{Spine switches} are the ``\textbf{top layer}''. These are high-capacity core switches. They \textbf{interconnect all the Leaf switches with each other}.
\end{itemize}
...
\highspace
\textcolor{Green3}{\faIcon{question-circle} \textbf{What problem does it solve?}} The classic 3-tier DCN designs (page \pageref{paragraph: classical Three-Tier architecture}) \textbf{don't scale bisection bandwidth linearly}. They easily become bottlenecks if many servers talk to each other. Leaf-Spine fixes this by:
...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{question-circle} \textbf{What is a Clos network, and how is it linked to leaf-spine architecture?}}
\end{flushleft}
Originally, the \definition{Clos network} was invented by Charles Clos (1952) for telephone switches. It's a \textbf{multi-stage switching fabric} that allows us to build a big non-blocking switch \textbf{out of many small cheap switches}. The core idea: instead of one giant switch, we \textbf{connect multiple small switches in stages} to get scalable bandwidth \textbf{with multiple paths}. In datacenter networking, the \textbf{Clos design} maps naturally to modern switch hardware.

\highspace
\textcolor{Green3}{\faIcon{check-circle} \textbf{Why is Clos good for datacenters?}} In huge datacenters:
\begin{itemize}
...
\highspace
\textcolor{Green3}{\faIcon{question-circle} \textbf{How does the Leaf-Spine topology come from Clos?}} \textbf{Leaf-Spine} is basically the \textbf{simplest practical version} of a 2-stage or 3-stage \textbf{Clos network}, tailored for Ethernet.

\highspace
\textcolor{Green3}{\faIcon{tools} \textbf{Formal Definition}}. A \textbf{Clos network} is a \textbf{multi-stage interconnection network} defined by three integers:
\begin{itemize}
...
\begin{itemize}
    \item The Clos network is \definitionWithSpecificIndex{rearrangeably non-blocking}{Clos network rearrangeably non-blocking condition}{} if: $m \geq n$ (number of middle-stage switches is greater or equal than number of inputs per input module). So \hl{we can rearrange connections to always find a path}.
    \item The Clos network is \definitionWithSpecificIndex{strictly non-blocking}{Clos network strictly non-blocking condition}{} if: $m \geq 2n - 1$ (the number of middle-stage switches is greater than or equal to twice the number of inputs per input module minus one).
\end{itemize}
We \textbf{mapped} the \textbf{Clos network} in data centers \textbf{to the leaf-spine} architecture:
\begin{itemize}
...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{question-circle} \textbf{Why do we scale to a 3-tier Clos?}}
\end{flushleft}
...
\end{itemize}
So to connect \emph{thousands} of racks at full bisection bandwidth, we need more stages. This is exactly what the \definition{Fat-Tree (or 3-stage Clos)} solves.

\highspace
A \definition{Fat-Tree (or 3-stage Clos)} is just a multi-level \textbf{Clos network} where link capacity \emph{increases} closer to the core, so the ``tree'' looks \textbf{fat} in the middle. The key idea is:
\begin{itemize}
...
\highspace
\textcolor{Green3}{\faIcon{question-circle} \textbf{How it works? 3-tier.}} A classic Fat-Tree has:
\begin{itemize}
    \item \important{Edge (Leaf)}: ToR switches in racks, connect to servers and Aggregation.
    \item \important{Aggregation (Middle)}: Aggregate Leafs, uplink to Core.
    \item \important{Core (Spone/Superspine)}: Connect Aggregations together.
\end{itemize}
...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{question-circle} \textbf{Why do we call it Pod-based?}}
\end{flushleft}
When we have \emph{thousands} of racks, it's unmanageable to think \emph{of one giant switch fabric}. So we \textbf{divide} the whole fabric \textbf{into repeated building blocks} called Pods. A \definition{PoD (Point of Delivery)}, in datacenter design, is a \textbf{modular}, \textbf{repeatable unit} of:
\begin{itemize}
...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{book} \textbf{A Scalable, Commodity Data Center Network Architecture \cite{al2008scalable}}}
\end{flushleft}
...
\begin{examplebox}[: Fat-Tree with $2k$ pods and $k=2$]
    ...
    \end{itemize}
    \begin{center}
        \phantomsection % creates a link target here
        \includegraphics[width=\textwidth]{img/fat-tree-compressed.pdf}
        \captionof{figure}{Simple fat-tree topology. Using the two-level routing tables, packets from source \texttt{10.0.1.2} to destination \texttt{10.2.0.3} would take the dashed path.\cite{al2008scalable}}
        \label{fig:fat-tree}
    \end{center}
...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{book} \textbf{CamCube: a concrete example}}
\end{flushleft}
\definition{CamCube} was an experimental \textbf{server-centric datacenter network} developed at the University of Cambridge around 2010. The key features are:
\begin{itemize}
...
\highspace
\textcolor{Green3}{\faIcon{question-circle} \textbf{Why do this?}}
\begin{itemize}
...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{question-circle} \textbf{What is DCell?}}
\end{flushleft}
\definition{DCell} is a \textbf{server-centric datacenter network} proposed by Microsoft Research (Guo et al., 2008). The key idea is build a large-scale datacenter network \textbf{without big switches}, using small commodity switches and \textbf{servers interconnecting themselves recursively}.
...
\begin{flushleft}
    \textcolor{Green3}{\faIcon{question-circle} \textbf{What is BCube?}}
\end{flushleft}
\definition{BCube} is a \textbf{server-centric datacenter network topology} proposed by Microsoft Research (Guo et al., 2009), same lab that proposed DCell. It was designed specifically for \textbf{modular datacenters}, think \textbf{container-based clusters} or shipping container datacenters.
...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=.7\textwidth]{img/networking-13.png}
