...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{question-circle} \textbf{Why use multiple datacenters?}}
\end{flushleft}
...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{sitemap} \textbf{The Hierarchical Organization of Multi-Datacenter Infrastructure}}
\end{flushleft}
...
\begin{enumerate}
    \item \definition{Geographic Areas (GAs)}. The \textbf{highest level of organization}, \hl{defined by geopolitical boundaries or country-specific regulations}. The primary factor is data residency laws, ensuring compliance with legal frameworks (e.g., GDPR in Europe).
    \begin{examplebox}[: Geographic Area]
    ...
    \item \definition{Computing Regions (CRs)}. A \textbf{finer granularity withing Geographic Areas}, representing \textbf{large-scale infrastructure within a geographic region}. \hl{Each GA contains at least two Computing Regions for redundancy}. Key characteristics:
    \begin{itemize}
    ...
    \begin{examplebox}[: Computing Region]
        AWS has ``US-East-1'' (Virginia) and ``US-West-1'' (California) as distinct computing regions.
    \end{examplebox}


    \item \definition{Availability Zones (AZs)}. Within a Computing Region, \textbf{datacenters are} further \textbf{divided into Availability Zones (AZs)}.
    \begin{itemize}
    ...
    \end{itemize}
\end{enumerate}
With the rise of low-latency applications, modern cloud providers go beyond central data centers by deploying \definition{Edge Locations}. They are \textbf{smaller facilities located closer to the user}. They are \textbf{used to cache} frequently accessed data and accelerate content delivery. Commonly used in content delivery networks (CDNs) such as Cloudflare, AWS CloudFront, and Akamai.

\begin{examplebox}[: Edge Locations]
    \begin{itemize}
...
\highspace
\definition{Availability} is measured as the \textbf{percentage of uptime over a given period} (usually a year). Another important topic is the \definition{Service Level Agreement (SLA)}, which is a \textbf{formal contract between a service provider} (e.g., AWS, Google Cloud, Microsoft Azure) \textbf{and a customer} that \hl{defines the expected level of service reliability, availability, and performance}. If the provider fails to meet the SLA, the customer may receive compensation (e.g., service credits or refunds).
...
\begin{itemize}
	\item \important{Servers (Compute Layer)}, section~\ref{subsubsection: Server (computation, HW accelerators)}, page~\pageref{subsubsection: Server (computation, HW accelerators)}.
    \begin{itemize}
	...
	\item \important{Storage Systems}, section~\ref{subsubsection: Storage (type, technology)}, page~\pageref{subsubsection: Storage (type, technology)}.
    \begin{itemize}
	...
	\item \important{Networking Infrastructure}, section~\ref{subsubsection: Networking (architecture and technology)}, page~\pageref{subsubsection: Networking (architecture and technology)}.
    \begin{itemize}
	...
	\item \important{Cooling Systems \& Environmental Control}, section~\ref{subsubsection: Cooling systems}, page~\pageref{subsubsection: Cooling systems}.
    \begin{itemize}
	...
	\item \important{Power Supply \& Energy Management}, section~\ref{subsubsection: Power supply}, page~\pageref{subsubsection: Power supply}.
    \begin{itemize}
	...
	\item \important{Failure Recovery \& Fault Tolerance}
    \begin{itemize}
