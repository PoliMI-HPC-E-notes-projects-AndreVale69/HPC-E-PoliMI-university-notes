...
\begin{itemize}
    ...
    \begin{equation}\label{eq: Availability of Series System}
...
\begin{figure}[!htp]
	...
	\begin{tikzpicture}[node distance=2cm]
        ...
        \node[
            ...
            minimum height=0.6cm,
            name=replica1
        ] at (3,1) {$\text{Module}_{1}$};
        \node[
            draw,
            minimum width=2cm,
            minimum height=0.6cm,
            name=replica2
        ] at (3,0) {$\text{Module}_{2}$};
        \node[
            draw,
            minimum width=2cm,
            minimum height=0.6cm,
            name=replica3
        ] at (3,-1) {$\text{Module}_{3}$};
        ...
        \draw[->] (voter) -- (output);
    \end{tikzpicture}
	\caption{General structure of TMR system.}
\end{figure}

\highspace
The \definition{TMR Reliability $R_{\text{TMR}}$} is:
\begin{equation}
    \begin{array}{rcl}
        R_{\text{TMR}} &=& R_{v}\left(3 \cdot R_{m}^{2} - 2 \cdot R_{m}^{3}\right) \\ [.5em]
                       &=& R_{v} \left[\left(\text{2-out-of-3 OK}\right) + \left(\text{3-out-of-3 OK}\right)\right]
    \end{array}
\end{equation}
Where:
\begin{itemize}
    \item $R_{m}$: reliability of a \textbf{module}.
    \item $R_{v}$: reliability of the \textbf{voter}.
    \item $3 \cdot R_{m}^{2}$: probability of exactly 2 modules working.
    \item $R_{m}^{3}$: probability of all 3 modules working.
\end{itemize}
...
\end{equation*}
And the \definition{MTTF of TMR system} is:
\begin{enumerate}
...
\highspace
\textcolor{Green3}{\faIcon{check-circle} \textbf{When TMR is Good}}
\begin{itemize}[label=\textcolor{Green3}{\faIcon{check}}]
...
\highspace
\textcolor{Red2}{\faIcon{times-circle} \textbf{When TMR is Bad}}. For \textbf{long mission durations}, TMR has a lower MTTF than a single module. So TMR is bad when long-term availability is key.

\highspace
\textcolor{Green3}{\faIcon{question-circle} \textbf{What is the MTTF Threshold?}} There exists a specific time $t^{*}$ where:
\begin{equation*}
...
\definition{Standby redundancy} is a fault-tolerance strategy where \textbf{redundant components are kept inactive} (in ``standby mode'') until the \textbf{primary component fails}, at which point a \textbf{backup} is activated.

\highspace
Unlike parallel redundancy, where all components run simultaneously, standby redundancy has \textbf{only one active component at a time}. It has two main roles:
\begin{enumerate}
    \item \important{Primary replica}: operates under normal conditions.
    \item \important{Standby (backup) replica(s)}: activated \textbf{only upon failure} of the primary.
\end{enumerate}
\begin{figure}[!htp]
	\centering
	\begin{tikzpicture}[node distance=2cm]
        ...
        \node[
            ...
            minimum height=0.6cm,
            name=replicaN-1
        ] at (3,-1) {$\text{Replica}_{n-1}$};
        \node[
            draw,
            minimum width=2cm,
            minimum height=0.6cm,
            name=replicaN
        ] at (3,-2) {$\text{Replica}_{n}$};
        ...
        \draw[->] (replicaDots) -- (switch);
        \draw[->] (6,2) -- (switch);
        \draw[->] (6,-2) -- (switch);

        \node[rectangle, name=output] at (8.5,0) {Output};

        \draw[->] (switch) -- (output);
    \end{tikzpicture}
	\caption{General standby redundancy strategy.}
\end{figure}
...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{question-circle} \textbf{Requirements for operation}}
\end{flushleft}
Standby redundancy only works if the system includes:
\begin{itemize}
    \item \important{Self-check mechanism}: Detects when the primary has failed.
    \item \important{Switch mechanism}: Transfers operation to the standby.
\end{itemize}
