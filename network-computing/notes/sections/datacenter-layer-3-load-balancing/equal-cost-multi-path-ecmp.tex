\subsection{Equal Cost Multi Path (ECMP)}\label{subsection: Equal Cost Multi Path (ECMP)}
...
\begin{itemize}
    \item \label{def: Bursty Traffic - Incast} \definition{Bursty Traffic (Incast)}. Incast happens when \textbf{one receiver asks for data from many servers at the same time}. For example, a storage node requests blocks from 50 servers; all 50 servers respond \textbf{at once} and their packets all converge on the \textbf{same final link} to the receiver.
...
hash(flow C) = 29 $\rightarrow$ 29 % 4 = 1 $\rightarrow$ path 2\end{lstlisting}
