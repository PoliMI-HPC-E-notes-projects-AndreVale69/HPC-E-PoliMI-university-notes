\subsection{Life of a Packet Inside a Server}\label{sec:life-of-a-packet-inside-a-server}
...
\highspace
\textcolor{Green3}{\faIcon{question} \textbf{What are packet buffers?}} \definition{Packet Buffers} \textbf{are regions of host\break (DRAM) memory allocated by the kernel used to store incoming packets}. For example, in Linux, these are typically \texttt{sk\_buff} structures that hold packet metadata and data, or memory pages managed by the networking subsystem. It is important to note that \hl{buffers are allocated \textbf{before} packets arrive because this avoids dynamic allocation on the fast path}.\footnote{%
...
\highspace
Putting it all together, an \definition{RX Descriptor Ring}\phantomsection\label{def:rx-descriptor-ring} is a \textbf{circular queue of descriptors that tell the NIC where to place incoming packets in host memory}. The RX descriptor ring is:
\begin{itemize}
