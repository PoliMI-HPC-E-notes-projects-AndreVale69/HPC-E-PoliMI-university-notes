...
\begin{itemize}
    \item \definition{Lexically Forward Dependency} occurs when the source statement (the one that produces the value) appears \textbf{before} the target statement (the one that consumes the value) within the same iteration of code. Essentially, it follows the natural top-to-bottom order of reading code.
...
for (i=0; i<5; i++) {
    ...
    \item \definition{Lexically Backward Dependency} occurs when the source statement comes \textbf{after} the target statement within the same iteration. It's a bit counterintuitive since it involves looking back to an earlier point in the code to resolve a dependency.
