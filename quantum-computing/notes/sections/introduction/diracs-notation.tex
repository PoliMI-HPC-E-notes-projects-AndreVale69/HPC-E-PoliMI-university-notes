...
\end{flushleft}
A \definition{Ket} $\ket{v}$ (is equal to the linear algebra annotation $\overrightarrow{v}$) is a \textbf{column vector} in a Hilbert space, that \textbf{represents a quantum state}.
\begin{equation*}
    \ket{v} = \begin{bmatrix}
        a \\ b
    \end{bmatrix}
...
\end{flushleft}
A \definition{Bra} $\bra{v}$ (is equal to the linear algebra annotation $\overrightarrow{v}^{H}$) is the \textbf{conjugate transpose (Hermitian conjugate) of the ket} $\ket{v}$.
...
\begin{itemize}
    ...
    \item \definitionWithSpecificIndex{Matrix-Ket Multiplication: $M\ket{v}$}{Dirac's Notation: Matrix-Ket Multiplication}{}
