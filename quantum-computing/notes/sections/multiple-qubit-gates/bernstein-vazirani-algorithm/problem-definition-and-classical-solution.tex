\subsection{Bernstein-Vazirani's Algorithm}\label{sec:bernstein-vazirani-algorithm}

\subsubsection{Problem Definition and Classical Solution}\label{sec:bernstein-vazirani-algorithm-problem-definition-and-classical-solution}
...
\highspace
\begin{flushleft}
    \textcolor{Green3}{\faIcon{bullseye} \textbf{The Hidden Bitstring (the goal)}}
\end{flushleft}
...
\end{flushleft}
The \hl{oracle implements the Boolean function:}
\begin{equation}\label{eq:bernstein-vazirani-oracle-function}
    f_w (x) = w \cdot x \mod 2
\end{equation}
Where:
\begin{itemize}
    \item[\textcolor{Green3}{\faIcon{check}}] $x \in \left\{0,1\right\}^{n}$ is an $n$-bit input string \textbf{chosen by us}.
    \item[\textcolor{Green3}{\faIcon{question-circle}}] $w$ is the \textbf{unknown} \emph{hidden bitstring} (Bernstein-Vazirani's goal).
...
\highspace
\textcolor{Red2}{\faIcon{hourglass-half}} Instead, a \hl{deterministic classical algorithm must determine every bit of $w$.} The simplest strategy is to query the oracle with inputs containing a single 1. For example:
\begin{equation*}
    x = 100 \dots 0
\end{equation*}
Then:
...
\begin{equation*}
    w = 1011
\end{equation*}
In general, an \hl{$n$-bit hidden string requires $n$ queries to the oracle to be fully determined by a \textbf{deterministic classical algorithm}.}
