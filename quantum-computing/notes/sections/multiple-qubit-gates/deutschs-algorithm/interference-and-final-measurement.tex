\subsubsection{Interference and Final Measurement}\label{ex:interference-and-final-measurement}
...
\begin{figure}[!htp]
    \centering
    \begin{quantikz}
        \lstick{\ket{0}}\slice{$\ket{\psi_{0}}$}    & \gate{H}\slice{$\ket{\psi_{1}}$}  & \gate[2][1.5cm]{U_f}\slice{$\ket{\psi_{2}}$}  & \gate{H}\slice{$\ket{\psi_{3}}$}  & \meter{}  & \\
        \lstick{\ket{1}}                            & \gate{H}                          &                                               &                                   &           &
    \end{quantikz}
    \caption{Deutsch's Algorithm. The final measurement of the data qubit reveals whether the function is constant or balanced.}
    \label{fig:deutsch-algorithm-circuit}
\end{figure}
