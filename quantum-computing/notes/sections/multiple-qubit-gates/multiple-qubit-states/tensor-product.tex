...
\begin{remarkbox}[: Inner Product of Tensor Products]
    For composite quantum systems (i.e., systems composed of multiple qubits), the \textbf{inner product between tensor product states factorizes}:
    \begin{equation}\label{eq:inner-product-tensor}
        \left(\bra{a} \otimes \bra{b}\right)\left(\ket{c} \otimes \ket{d}\right) = \braket{a}{c} \cdot \braket{b}{d}
    \end{equation}
    This means that the ``similarity'' (overlap) between two composite states is the product of the similarities of their individual subsystems (\autopageref{eq:inner-product}).

    \highspace
    For example:
    \begin{equation*}
        \left(\bra{0} \otimes \bra{+}\right)\left(\ket{0} \otimes \ket{-}\right)
    \end{equation*}
    Has overlap:
    \begin{equation*}
        \braket{0}{0} \braket{+}{-} = 1 \cdot 0 = 0
    \end{equation*}
    So even though the first qubit is identical, the total states are orthogonal because the second qubits are orthogonal!

    \highspace
    \textcolor{Green3}{\faIcon[regular]{lightbulb} \textbf{Intuition.}} A tensor product state combines independent subsystems. Therefore, the total overlap must take into account the overlap of \emph{each} subsystem simultaneously. \hl{If one subsystem is perfectly distinguishable (overlap $0$), then the entire composite states become perfectly distinguishable.}
\end{remarkbox}

\begin{flushleft}
    \textcolor{Red2}{\faIcon{exclamation-triangle} \textbf{Not all states can be factored}}\phantomsection\label{ex:not-all-states-can-be-factored}
\end{flushleft}
...
\begin{itemize}
    ...
    \end{equation*}
    Since $c_{0}c_{3} \neq c_{1}c_{2}$, the state is not a tensor product and thus is \textbf{not separable}. This state is an example of an \textbf{entangled state} (\definition{Entanglement} phenomenon), where the two qubits are correlated in such a way that we cannot describe them independently. We will see more about entanglement in the next section, but for now, the key takeaway is that not all multi-qubit states can be factored into tensor products of single-qubit states, and this is a fundamental aspect of quantum mechanics that leads to phenomena like entanglement.
\end{itemize}
