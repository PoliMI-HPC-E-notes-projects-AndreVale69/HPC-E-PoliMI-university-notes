...
\begin{remarkbox}[: Explain $H\ket{x} = \frac{1}{\sqrt{2}} \sum_{y\in\{0,1\}} (-1)^{xy}\ket{y}$]\phantomsection\label{rem:hadamard-formula}
...
\end{equation*}
That \hl{$y$ is guaranteed to satisfy:}
\begin{equation}\label{eq:simons-algorithm-measurement-constraint}
    a \cdot y = 0 \mod 2
\end{equation}
