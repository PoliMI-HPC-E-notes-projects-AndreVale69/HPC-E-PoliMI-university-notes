...
\begin{figure}[!ht]
    \centering
    \tikzsetnextfilename{transverse-field-ising-model-axes}
    \begin{tikzpicture}[
        scale=1.2,
        axis/.style={->, thick},
...
\begin{itemize}
    ...
    \begin{equation*}
        \frac{B(s)}{2} \underbrace{\left( \sum_i h_i \hat{\sigma}_{z}^{(i)} + \sum_{i > j} J_{ij} \hat{\sigma}_{z}^{(i)} \hat{\sigma}_{z}^{(j)} \right)}_{H_P}
    \end{equation*}
...
\end{itemize}
Frequently, the annealing Hamiltonian is written in a more \textbf{compact form}:
\begin{equation}
    H(s) = A(s) H_A + B(s) H_P
\end{equation}
...
\begin{figure}[!htp]
    \centering
    \tikzsetnextfilename{quantum-annealing-schedule-weights}
    \begin{tikzpicture}[
        scale=1.1,
        axis/.style={->, thick},
        curveA/.style={thick},
