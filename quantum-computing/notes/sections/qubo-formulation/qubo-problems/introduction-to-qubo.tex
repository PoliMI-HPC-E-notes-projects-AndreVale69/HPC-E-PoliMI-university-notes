...
\end{flushleft}
The general QUBO formulation is\footnote{%
    The coefficients $a_{i}$ and $q_{ij}$ are sometimes called \textbf{biases} and \textbf{couplers}, respectively. The origin of this terminology will become clear when introducing the Ising model (\autopageref{sec:classical-ising-model}).
}:
\begin{equation}\label{eq:qubo-formulation}
