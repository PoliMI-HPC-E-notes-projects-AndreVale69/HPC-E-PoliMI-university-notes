...
\begin{itemize}
    ...
    \begin{equation*}
        \sum_i h_i s_i
    \end{equation*}
    The coefficient $h_i$ represents an external magnetic field acting on spin $i$. We can think of it as a preference (also called ``\emph{bias}'', because it biases the spin toward a particular direction). This \textbf{coefficient encourages the spin to align in a certain direction}:
    \begin{itemize}
    ...
    \end{equation*}
    This term describes \textbf{how pairs of spins influence each other}. The coefficient $J_{ij}$ is called the \textbf{coupling strength} between spins $i$ and $j$ (also called ``\emph{coupler}'', because it \emph{couples} the spins together and creates a connection between qubits).
    \begin{itemize}
        ...
        \item[\textcolor{Green3}{\faIcon{question-circle}}] \textcolor{Green3}{\textbf{What does $J_{ij}$ represent?}} The coefficient $J_{ij}$ represents the\break strength of the interaction between spins $i$ and $j$.
