...
\end{flushleft}
From \autoref{eq:rotation-operator-general} (\autopageref{eq:rotation-operator-general}) and \autoref{eq:rotation-operator-general-expanded} (\autopageref{eq:rotation-operator-general-expanded}), a rotation around the $y$-axis by an angle $\theta$ is given by:
\begin{equation*}
...
\begin{equation*}
    R_y(\pi) = -iY
\end{equation*}
Therefore, the Pauli-Y gate corresponds to a \textbf{rotation of $\pi$ around the $y$-axis, up to a global phase}:
\begin{equation*}
    Y \equiv R_y(\pi)
\end{equation*}
...
\begin{figure}[!htp]
    ...
    \tikzsetnextfilename{pauli-y-rotation-bloch-sphere}
