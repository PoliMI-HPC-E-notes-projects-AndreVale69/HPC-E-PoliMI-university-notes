...
\begin{figure}[!htp]
    ...
    \tikzsetnextfilename{pauli-z-rotation-bloch-sphere}
...
\end{flushleft}
From \autoref{eq:rotation-operator-general} (\autopageref{eq:rotation-operator-general}) and \autoref{eq:rotation-operator-general-expanded} (\autopageref{eq:rotation-operator-general-expanded}), a rotation around the $z$-axis by an angle $\theta$ is given by:
\begin{equation*}
...
\begin{equation*}
    R_z(\pi) = -iZ
\end{equation*}
Therefore, the Pauli-Z gate corresponds to a \textbf{rotation of $\pi$ around the $z$-axis, up to a global phase}:
\begin{equation*}
    Z \equiv R_z(\pi)
\end{equation*}
