...
\begin{equation*}
    \ket{\psi} = a \ket{0} + b \ket{1}
\end{equation*}
Represents a \textbf{valid state} of a qubit. Some \hl{\textbf{properties} of the qubit state} are:
\begin{itemize}
    ...
    \begin{definitionbox}[: Normalized Quantum State]\phantomsection\label{def:normalized-quantum-state}
        A quantum state $\ket{\psi}$ is said to be \definitionWithSpecificIndex{normalized}{Normalized quantum state}{} if its norm is equal to one:
        \begin{equation*}
            \left\|\ket{\psi}\right\| = 1
        \end{equation*}
