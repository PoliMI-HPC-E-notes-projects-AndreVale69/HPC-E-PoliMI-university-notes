\subsection{State Space of a Single-Qubit System}\label{sec:state-space-of-a-single-qubit-system}
...
\begin{itemize}
    ...
    \begin{figure}[!htp]
        \centering
        \resizebox{\linewidth}{!}{
            \tikzsetnextfilename{normalization-constraint-spheres}
    ...
    \item \phantomsection\label{sec:global-phase-equivalence} \important{Global Phase Equivalence}. Two states that differ only by a \textbf{global phase} represent the \textbf{same physical qubit}:
    \begin{equation*}
...
\begin{enumerate}
    ...
    \end{equation*}
    Where we define also the \definition{Relative Phase} $\phi = \beta - \alpha$. At this point, the state is characterized by two real magnitudes $\left|a\right|$ and $\left|b\right|$, and one relative phase $\phi$. Mathematically, if:
    \begin{equation*}
        a = \left|a\right| e^{i\alpha} \qquad b = \left|b\right| e^{i\beta}
    \end{equation*}
    Then:
    \begin{equation}
        \text{Relative Phase} = \phi = \arg\left(b\right) - \arg\left(a\right) = \beta - \alpha
    \end{equation}
...
\begin{itemize}
    \item $\theta$: controls the \hl{relative weights of $\ket{0}$ and $\ket{1}$} in the superposition, determining the \hl{probabilities of measuring each state.}
    \item $\phi$: controls the \hl{relative phase between the $\ket{0}$ and $\ket{1}$} components. While it does not affect measurement probabilities, it is crucial for interference effects and the behavior of the qubit under quantum gates (which we will discuss later).
\end{itemize}
...
\begin{itemize}
    ...
    \begin{equation*}
        \ket{0} = \cos\left(\dfrac{0}{2}\right)\ket{0} + e^{i\phi}\sin\left(\dfrac{0}{2}\right)\ket{1} = 1 \cdot \ket{0} + 0 \cdot \ket{1} = \ket{0}
    \end{equation*}
    \item $\ket{1}$ corresponds to the south pole, since $\theta = \pi$:
    \begin{equation*}
        \ket{1} = \cos\left(\dfrac{\pi}{2}\right)\ket{0} + e^{i\phi}\sin\left(\dfrac{\pi}{2}\right)\ket{1} = 0 \cdot \ket{0} + e^{i\phi} \cdot 1 \cdot \ket{1} = e^{i\phi} \ket{1} \Rightarrow \ket{1}
    \end{equation*}
    \textcolor{Red2}{\faIcon{exclamation-triangle} \textbf{Attention!}} $e^{i\phi}\ket{1}$ becomes $\ket{1}$ after \textbf{removing the global phase} (physically unobservable), so $\ket{1}$ is indeed at the south pole. In general, all states of the form $e^{i\phi}\ket{1}$ represent the \textbf{same physiical state} as $\ket{1}$. Hence, $\ket{1}$ corresponds to the \textbf{south pole} of the Bloch sphere.
...
\begin{figure}[!htp]
    ...
    \tikzsetnextfilename{bloch-sphere-canonical-representation}
    ...
    \end{tikzpicture}
    \caption{Bloch sphere representation of a pure qubit state. The red vector $\ket{v}$ represents a generic qubit state. Measuring $\ket{v}$ along the $z$-axis gives probabilities determined by $\theta$, while along $x$ or $y$-axes reveals information about the relative phase $\phi$.}
    \label{fig:bloch-sphere}
\end{figure}

\noindent
On the Bloch sphere, the axes correspond to \textbf{three fundamental measurement bases} for a qubit. Each pair of opposite points represents the \textbf{eigenstates of one of the Pauli operators} (we will discuss the Pauli operators in more detail in \ref{sec:pauli-x-not-gate}, \autopageref{sec:pauli-x-not-gate}):
\begin{itemize}
    \item \important{$z$-axis}, \textbf{computational basis} $\{\ket{0}, \ket{1}\}$, eigenstates of $\sigma_{z}$. The states are:
    \begin{equation*}
        \ket{0} = \begin{pmatrix}
            1 \\ 0
        \end{pmatrix}
    ...
    \begin{itemize}
        \item $\ket{0}$: North pole of the Bloch sphere $\left(0, 0, 1\right)$.
        \item $\ket{1}$: South pole of the Bloch sphere $\left(0, 0, -1\right)$.
    \end{itemize}
    Measuring along the $z$-axis answer the question: ``\emph{is the qubit $\ket{0}$ or $\ket{1}$?}''.


    \item \important{$x$-axis}, \textbf{Hadamard basis} $\{\ket{+}, \ket{-}\}$, eigenstates of $\sigma_{x}$. The states are:
    \begin{equation*}
    ...
    \end{equation*}
    \begin{itemize}
        \item $\ket{+}$: $+x$ direction on the equator $\left(1, 0, 0\right)$, with $\phi = 0$ (no relative phase).
        \item $\ket{-}$: $-x$ direction on the equator $\left(-1, 0, 0\right)$, with $\phi = \pi$ (relative phase of $\pi$ between $\ket{0}$ and $\ket{1}$).
    \end{itemize}
    These states represent \textbf{equal superpositions} of $\ket{0}$ and $\ket{1}$ with \textbf{relative phase} $0$ and $\pi$.


    \item \important{$y$-axis}, \textbf{imaginary basis} $\{\ket{i}, \ket{-i}\}$, eigenstates of $\sigma_{y}$. The states are:
    \begin{equation*}
    ...
    \end{equation*}
    \begin{itemize}
        \item $\ket{i}$: $+y$ direction on the equator $\left(0, 1, 0\right)$, with $\phi = \dfrac{\pi}{2}$ (relative phase of $\dfrac{\pi}{2}$ between $\ket{0}$ and $\ket{1}$).
        \item $\ket{-i}$: $-y$ direction on the equator $\left(0, -1, 0\right)$, with $\phi = -\dfrac{\pi}{2}$ (relative phase of $-\dfrac{\pi}{2}$ between $\ket{0}$ and $\ket{1}$).
    \end{itemize}
    These states also have equal probabilities of measuring $\ket{0}$ or $\ket{1}$ in the computational basis, but they differ by a \textbf{relative phase of} $\pm \frac{\pi}{2}$. This phase is crucial for \textbf{interference effects} in quantum algorithms.
\end{itemize}