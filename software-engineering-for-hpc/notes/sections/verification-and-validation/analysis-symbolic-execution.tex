...
\highspace
When we are talking about test cases, it's necessary to introduce \definition{Unit Testing}. It is \textbf{conducted by developers} and \textbf{aims to test small pieces} (units) \textbf{of code in isolation}. But when we test in isolation, there should be a \textbf{problem}: the \textbf{units may depend on other units}. Then, we need to simulate missing units.

\begin{flushleft}
    \textcolor{Red2}{\textbf{Integration Testing}}
\end{flushleft}
The \definition{Integration Testing} is \textbf{aimed at exercising the interaction between interfaces and components}. The \textbf{faults} are discovered by integration testing, such as:
\begin{itemize}
